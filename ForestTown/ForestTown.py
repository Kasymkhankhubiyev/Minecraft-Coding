...
#backwall
backlength = west_wall_length+east_wall_length+towersdist+ 2* tower_length
scwall.WestPropWall(x+7, pos.y, pos.z+(tower_length + 2)+north_wall_length+1, backlength , tower_height - 2, tower_width - 2)
...
for i in range(step, east_wall_length, step): #вправо -x
    if (east_wall_length - (i + 5*(i/step))) >= step:
        buildCastle(pos.x-i-5*(i/step), pos.y, pos.z, tower_height, tower_width, tower_length, 0)

for i in range(step, west_wall_length, step): #влево +x
    if (west_wall_length - (i + 5*(i/step))) >= step:
        buildCastle(pos.x+2*tower_length+towersdist+i, pos.y, pos.z, tower_height, tower_width, tower_length, 0)
        
#backside
for i in range(step, backlength, step): #влево +x
    if (backlength - (i + 5*(i//step))) >= step:
        buildCastle(pos.x-east_wall_length+i+5*(i//step), pos.y, pos.z+7+north_wall_length, tower_height, tower_width, tower_length, 1)
